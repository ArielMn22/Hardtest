...
echo "[$(date)] Menu RAM teste de velocidade iniciado" >> "$DIRET/.MANAGE/log.txt"

pulalinha(){ echo ; } #função para pular linha

#erro(){ dialog --stdout --title 'sair' --msgbox 'falouuuu!!' 0 0 ; exit 0 ; }
...
dois=$(echo "arquivo criado foi lido em memória RAM com velocidade de leitura de foi de $leituraum" >> '.textoRAM.txt' )
...
quatro=$(echo "arquivo criado foi lido em memória RAM com velocidade de leitura de foi de $leituradois" >> '.textoRAM.txt' )
	$(echo " " >> .textoRAM.txt) #pula linha no arquivo de texto
	$(rm '.staging1024E.txt')
	$(rm '.staging1024L.txt')
...
seis=$(echo "arquivo criado foi lido em memória RAM com velocidade de leitura de foi de $leituratres" >> '.textoRAM.txt' )
	$(rm '.staging2048E.txt')
	$(rm '.staging2048L.txt') 
...
QDZLGREP=$(cat '.staging512L.txt' | grep bytes | cut -d" " -f10,11 )
	$(echo "arquivo criado foi lido em memória RAM com velocidade de leitura de foi de $QDZLGREP" >> '.textoRAM.txt' )
...
MVQLGREP=$(cat '.staging1024L.txt' | grep bytes | cut -d" " -f10,11 )
	$(echo "arquivo criado foi lido em memória RAM com velocidade de leitura de foi de $MVQLGREP" >> '.textoRAM.txt' )
...
DQOLGREP=$(cat '.staging2048L.txt' | grep bytes | cut -d" " -f10,11 )
	$(echo "arquivo criado foi lido em memória RAM com velocidade de leitura de foi de $DQOLGREP" >> '.textoRAM.txt' )
...
case $menu in
    ...
    6) leituraDQO ;;
    7) conjunto ;;
    8) echo "[$(date)] Menu RAM teste de velocidade encerrado" >> "$DIRET/.MANAGE/log.txt" ; . "$DIRET/RAM/menuprojeto.sh" ;;
		*) . "$DIRET/.menu.sh" ;; 
esac
}
MENUP
